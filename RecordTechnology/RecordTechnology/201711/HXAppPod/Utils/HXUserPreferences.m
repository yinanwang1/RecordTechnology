...
#import "HXUserPreferences.h"

#define SETTINGS_KEY @"hx.usersSettings"
#define SETTINGS_CURRENT_USER @"hx.currentSettingsUser"
#define DEFAULT_USER @"_DefaultUser"

static HXUserPreferences *__sharedHXUserPreference = nil;

@interface HXUserPreferences()

@property(nonatomic, strong) NSString *currentUser;

@end

@implementation HXUserPreferences

@synthesize currentUser = _currentUser;

+(HXUserPreferences *)sharedInstance {
    if (__sharedHXUserPreference == nil) {
        __sharedHXUserPreference = [[HXUserPreferences alloc] init];
        NSString *user = [[NSUserDefaults standardUserDefaults] objectForKey:SETTINGS_CURRENT_USER];
        if (user == nil)
            user = DEFAULT_USER;
        __sharedHXUserPreference.currentUser = user;
    }
    return __sharedHXUserPreference;
}
